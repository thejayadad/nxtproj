...
export async function PUT(req, ctx) {
    ...
    try {
        ...
        if (post?.authorId?._id.toString() !== decodedToken._id.toString()) {
            return new Response(JSON.stringify({ msg: 'Only author can update post' }), { status: 403 })
        }
...
export async function DELETE(req, ctx) {
    ...
    try {
        const post = await Post.findById(id).populate('authorId')
        if (post?.authorId?._id.toString() !== decodedToken._id.toString()) {
            return new Response(JSON.stringify({ msg: 'Only author can delete this post' }), { status: 403 })
        }
